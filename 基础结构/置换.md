...
mod permutation {
    ...
    sort_split(sym: Sym[n], i: 0..n) :: Sym[n] = sym.filter($ < i) + sym.filter($ >= i)
    ...
    min_splits_sort(sym: Sym[n])
        ...
        = gen { li: powerset(0..n) => li.all(disordered_in(sym, $)) }.map($.len()).min()
}
```

[CodeForces-1863B](https://codeforces.com/problemset/problem/1863/B)
