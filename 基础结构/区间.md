...
```rs
mod interval {
    Interval = (s: ℕ, e: ℕ) where s <= e

    min_cover(intvls: Interval*) 
        = gen { li: ℕ* => intvls.all(i => li.some($ in i)) }.map($.len()).min()
        = min_cover_inner(
            $.filter(!$.is_empty())
    ...
    min_cover_inner(intvls + [i])
        // i 必被选
        = intvls.subseqs().filter(li =>
            intvls.all(intvl => li.some($.s in intvl) || i.s in intvl) && i.s in i
        ).map($.len()).min()
